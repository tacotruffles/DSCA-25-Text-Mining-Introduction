...
tidy_books <- tidy_books %>%
  anti_join(stop_words) # remove stop
...
```{r}
hgwells <- gutenberg_download(c(35, 36, 5230, 159)) # HG Wells indiceis
```
...
```{r}
bronte <- gutenberg_download(c(1260, 768, 969, 9182, 767)) # BRont's books
```
...
frequency <- bind_rows(mutate(tidy_bronte, author = "Brontë Sisters"),
  ...
  count(author, word) %>%
  group_by(author) %>%
  mutate(proportion = n / sum(n)) %>%  # how frequent a word was used
  select(-n) %>% 
